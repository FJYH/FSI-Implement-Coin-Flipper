...
document.addEventListener('DOMContentLoaded', function () {
    //TO DO: Add event listener and handler for flip and clear buttons
    // document.querySelector('flip')

    
    // Flip Button Click Handler
    document.getElementById('flip').addEventListener('click', () => {
        //Determine the results using Math.random(), 0 - 1
        let flippedHeads = Math.random() < 0.5;
        
        if (flippedHeads) {
            // Display the image and message as heads
            document.getElementById('penny-image').src = "assets/images/penny-heads.jpg"
            document.getElementById('message').textContent = 'You Flipped Heads!';

            // Increment the headsRoll counter
            headsRolls++;
        } else {
            // Display the image and message as tails
